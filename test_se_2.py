...
file_log = 'data_train\\report\\test_{}_bagging_booting_2.log'
...
i=0
j=0
with open(file_input, "r", encoding="utf-8") as t_file:
    input_file = t_file.read()
    for name_models in range(0,3):
        i=i+1
        print(i)
        print("Bt_matrix_T:{}".format(Bt_matrix_T[name_models]))
        print("Bt_test_T:{}".format(Bt_test_T[name_models]))
        for t in range(0,len(Bt_matrix_T[name_models])):
        
            
            j=j+1
            print("_{}".format(j))
            if(Bt_matrix_T[name_models][t] != Bt_test_T[name_models][t]):
                TNN.update_weights_models(name_models,input_file[t] , Bt_test_T[name_models][t])
